...
<f:verbatim>
  ...
  var djConfig = {
    ...
    $(document).ready(function(){
      ...
      dpCfg.attach("frmSearchCriteria:scContentDateFrom");
      dpCfg.attach("frmSearchCriteria:scContentDateTo"); 
    });
...
//      elTable.className = "noneSelectedResultRow ";
  ...
  function scReadRestUrlParams() {
    ...
    if (scText != "")
    	searchTerms.push(scText);
        
    /////////////////////////////////////
    //Update the additional options that were previously in the popup window, but are now on the main page
    scUpdateAdditionalOptions(false);
        
    var startDate = GptUtils.valChkStr(dojo.byId('frmSearchCriteria:scContentDateFrom').value);
    if (startDate != "")
    	searchTerms.push("endDate:[" + startDate + " TO 2100-01-01]");
    
    var endDate = GptUtils.valChkStr(dojo.byId('frmSearchCriteria:scContentDateTo').value);
    if (endDate != "")
    	searchTerms.push("startDate:[1800-01-01 TO " + endDate + "]");            
    
    var origin = GptUtils.valChkStr(dojo.byId('frmSearchCriteria:scOriginText').value);
    if (origin != "")
    	searchTerms.push('origin:"' + origin + '"');
    
    //var distribution = GptUtils.valChkStr(dojo.byId('frmSearchCriteria:scDistributionText').value);
    //if (distribution != "")
    //	searchTerms.push('distribution:"' + distribution + '"');
    
   	if(dojo.byId('frmSearchCriteria:chkThemeKey1').checked)
		searchTerms.push('dataTheme:' + '"Oceans > Bathymetry/Seafloor Topography"');
   	
	if(dojo.byId('frmSearchCriteria:chkThemeKey2').checked)
		searchTerms.push('dataTheme:' + '"Land Surface > Topography"');
	
	if(dojo.byId('frmSearchCriteria:chkThemeKey3').checked)
		searchTerms.push('dataTheme:' + '"Digital Elevation Model"');
	
	if(dojo.byId('frmSearchCriteria:chkThemeKey4').checked)
		searchTerms.push('dataTheme:' + '"LIDAR"');
    
    var resType = dojo.byId('frmSearchCriteria:scSelResource').value;
    if (resType!=("")) searchTerms.push(resType + ':' + '*');
        
    if(searchTerms.length > 0) {
      restParams += "&searchText=" +  encodeURIComponent(searchTerms.join(' AND '));
    }
    ...
    if(scContentType != "") {    	
      alert(encodeURIComponent(scContentType));
    }
...
<h:outputText escape="false" value="<h4><i>"/>
<h:outputText  value="Search metadata content, including title, abstract, and keywords"/>
<h:outputText escape="false" value="<br/>"/>
<h:outputText escape="false" value="</i><a href=\"/geoportal/webhelp/en/geoportal/index.html#/Using_Lucene_Search_Text_Queries/00t000000004000000/\" target=\"_blank\">(More information about query syntax)</a>"/>
<h:outputText escape="false" value="</h4>"/>
...
<h:panelGrid columns="4">
  <h:inputText id="scText"
               ...
               maxlength="4000" style="width:240px;" styleClass="searchBox" />
  ...
  <h:commandButton id="btnClearSearch" rendered="true" value="Clear All"
...
<h:outputText id="scLblSptial" value="Geographic Area" />
<h:outputText escape="false" value="</h3>"/>
<h:outputText escape="false" value="<h4><i>"/>
<h:outputText value="Zoom the map to desired area, and choose \"intersecting\" or \"fully within\"" />
<h:outputText escape="false" value="</i></h4>"/>
<h:panelGrid columns="1" id="scPnlSpatial">
  ...
  <h:panelGrid id="pnlMap">
	...
	<f:verbatim>
		<br/>
		<div dojoType="dijit.TitlePane" class="section" title="<u><b>Additional Search Criteria</b></u>" open="false">
	</f:verbatim>
	-->        	
			<h:outputText escape="false" value="<h2>"/>
			<h:outputText  value="Additional Search Criteria"/>
			<h:outputText escape="false" value="</h2>"/>
			<!--<h:outputText escape="false" value="<br/>"/>-->
			<% // modification date %>
			<h:panelGroup id="_pngContentDateSection">
  				<h:outputText escape="false" value="<h3>"/>
  				<h:outputText id="scLblContentDate" value="Date of Content" />
  				<h:outputText escape="false" value="</h3>"/>
  				<h:panelGrid columns="3" id="scPnlContentDate">
    				<h:outputLabel id="scLblContentDateFrom" for="scContentDateFrom"
                   		value="#{gptMsg['catalog.search.filterTemporal.dateFrom']}" />                   		
    				<h:inputText id="scContentDateFrom" maxlength="10" />
    				<h:outputText value="(yyyy-mm-dd)" />
    				<h:outputLabel id="scLblContentDateTo" for="scContentDateTo"
                   		value="#{gptMsg['catalog.search.filterTemporal.dateTo']}" />
    				<h:inputText id="scContentDateTo" maxlength="10"/>
    				<h:outputText value="(yyyy-mm-dd)" />
  				</h:panelGrid>
			</h:panelGroup>
			<h:outputText escape="false" value="<h3>"/>
			<h:outputText  value="Theme Keywords include:"/>
			<h:outputText escape="false" value="</h3>"/>
			<h:panelGroup>					
			<h:panelGrid columns="2" id="scPnlThemeKeywords">
  				<!--<h:selectManyCheckbox id="scSelThemeKeywords" layout="pageDirection">-->  		
        		<h:selectBooleanCheckbox id="chkThemeKey1" />
        		<h:outputText value="Oceans > Bathymetry/Seafloor Topography"/>
        		<h:outputText escape="false" value="<br/>"/>
        		
        		<h:selectBooleanCheckbox id="chkThemeKey2" />
        		<h:outputText value="Land Surface > Topography"/>
        		<h:outputText escape="false" value="<br/>"/>
        		
        		<h:selectBooleanCheckbox id="chkThemeKey3" />
        		<h:outputText value="Digital Elevation Model"/>
        		<h:outputText escape="false" value="<br/>"/>
        		
        		<h:selectBooleanCheckbox id="chkThemeKey4" />
        		<h:outputText value="LIDAR"/>
        		<h:outputText escape="false" value="<br/>"/>
        		        		
  				<!--</h:selectManyCheckbox>-->
  			</h:panelGrid>
  			</h:panelGroup>
  			<!-- <h:outputText  value="Placename keyword (example: Pacific, \"New Jersey\"):"/>
  			<h:outputText escape="false" value="<br/>"/>
			<h:inputText id="scPlacenameText" maxlength="4000" styleClass="searchBox" /> -->
			<h:outputText escape="false" value="<br/>"/>
			<h:outputText  value="Originating Organization (example: NOAA, USGS, USACE):"/>
			<h:outputText escape="false" value="<br/>"/>
			<h:inputText id="scOriginText" maxlength="4000" styleClass="searchBox" />
			<h:outputText escape="false" value="<br/>"/>
			<!-- <h:outputText  value="Distribution Organization (example: NGDC, USGS):"/>
			<h:outputText escape="false" value="<br/>"/>
			<h:inputText id="scDistributionText" maxlength="4000" styleClass="searchBox" /> -->
			<h:outputText escape="false" value="<br/>" />
			<% // modification date %>
			<h:panelGroup id="_pngModDateSection">
				<h:outputText escape="false" value="<h3>" />
				<h:outputText id="scLblDate"
					value="Metadata Modification Date:" />
				<h:outputText escape="false" value="</h3>" />
...
<!-- //NGDC Mods 10/31/2011 -->
<h:panelGroup id="_pngCtypeLocal">

  <h:outputText escape="false" value="<h3>"/>
  <h:outputText id="scLblResource" value="RESOURCE TYPE" />
  <h:outputText escape="false" value="</h3>"/>
  
  <h:panelGrid id="scPnlResource">
    <h:selectOneMenu id="scSelResource"
                     value=""
                     onchange=""
                     >
      <f:selectItem itemValue=""
                    itemLabel="" />                     
      <f:selectItem itemValue="wms.resource.url"
                    itemLabel="WMS" />
      <f:selectItem itemValue="wcs.resource.url"
                    itemLabel="WCS" />
      <f:selectItem itemValue="sos.resource.url"
                    itemLabel="SOS" />                    
      <f:selectItem itemValue="odp.resource.url"
                    itemLabel="OpenDAP" />
      <f:selectItem itemValue="ags.resource.url"
                    itemLabel="ArcGIS" />
    </h:selectOneMenu>
  </h:panelGrid>
</h:panelGroup>
...
<h:inputHidden id="scSelThemeKeywordsHidden" >
  <f:converter  converterId="gpt.ListToString" />
</h:inputHidden>
